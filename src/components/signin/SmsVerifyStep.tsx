...
export default function SmsVerifyStep() {
  ...
  const handleCodeInput = (index: number, value: string) => {
    ...
    if (value && index < 5) {
      const nextInput = document.getElementById(`signin-code-${index + 1}`);
      nextInput?.focus();
    }
  };
  ...
  const handleBackspace = () => {
    ...
    if (lastFilledIndex !== -1) {
      ...
      const input = document.getElementById(`signin-code-${lastFilledIndex}`);
      input?.focus();
    }
  };
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      updateSigninData({ verificationCode: enteredCode });
  ...
  const maskedPhone = signinData.phoneNumber.slice(0, -3) + '***';
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Backspace' && !digit && index > 0) {
                    const prevInput = document.getElementById(`signin-code-${index - 1}`);
                    prevInput?.focus();
                  }
                }}

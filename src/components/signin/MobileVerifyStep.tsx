...
export default function MobileVerifyStep() {
  const { signinData, updateSigninData, nextStep, previousStep, setIsLoading, currentFlow } = useSignin();
  ...
  const handlePhoneChange = (value: string) => {
    ...
    updateSigninData({ phoneNumber: formatted });
  };

  const handleSendCode = async () => {
    const phoneDigits = signinData.phoneNumber.replace(/\D/g, '');
  ...
  const isPhoneValid = signinData.phoneNumber.replace(/\D/g, '').length >= 10;

  // Use existing phone for login flow, empty for forgot password flow
  const initialPhone = currentFlow === 'login' ? '+20 10 1234 5678' : '';
  
  // Set initial phone if not already set
  React.useEffect(() => {
    if (currentFlow === 'login' && !signinData.phoneNumber) {
      updateSigninData({ phoneNumber: initialPhone });
    }
  }, [currentFlow, signinData.phoneNumber, updateSigninData, initialPhone]);
